...
from telegram_bot.handlers.account_handlers import (
    ...
    add_account_cookie_handler, enter_username_cookie, enter_cookies, 
    confirm_add_cookie_account, cancel_add_cookie_account,
    ENTER_USERNAME_COOKIE, ENTER_COOKIES, CONFIRM_COOKIE_ACCOUNT
)
...
/add_account_cookie - Добавить аккаунт по cookies
...
/cancel - Отменить текущую операцию
    ...
    elif query.data == 'add_account_cookie':
        # Вызываем обработчик добавления аккаунта по cookies
        return add_account_cookie_handler(update, context)
...
def setup_bot(updater):
    ...
    # Добавляем ConversationHandler для загрузки аккаунтов по cookies
    cookie_account_conversation = ConversationHandler(
        entry_points=[
            CommandHandler("add_account_cookie", add_account_cookie_handler),
            CallbackQueryHandler(add_account_cookie_handler, pattern='^add_account_cookie$')
        ],
        states={
            ENTER_USERNAME_COOKIE: [MessageHandler(Filters.text & ~Filters.command, enter_username_cookie)],
            ENTER_COOKIES: [MessageHandler(Filters.document.file_extension("json"), enter_cookies)],
            CONFIRM_COOKIE_ACCOUNT: [
                CallbackQueryHandler(confirm_add_cookie_account, pattern='^confirm_add_cookie_account$'),
                CallbackQueryHandler(cancel_add_cookie_account, pattern='^cancel_add_cookie_account$')
            ]
        },
        fallbacks=[CommandHandler("cancel", cancel_handler)]
    )
    dp.add_handler(cookie_account_conversation)
